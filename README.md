...
下面每个“截图占位区”就是页面截图在 README 中的放置位置。图片文件统一放在 [`demo/`](demo/README.md) 目录；请用 Google Chrome 截取完整浏览器页面后，覆盖对应的同名 PNG 文件，README 会自动展示实际页面。

### 作品运营

**截图占位区 1：作品管理**

替换文件：`demo/pc-work-list.png`

![作品管理：筛选、批量操作、分类和封面](demo/pc-work-list.png)

作品管理统一查看作品、作者、分类、连载状态、审核状态、章节数和横竖版封面，并提供批量上架/下架与作品详情入口。

**截图占位区 2：内容审核**

替换文件：`demo/pc-audit.png`

![内容审核：正文预览与审核记录](demo/pc-audit.png)

内容审核承接采集中心产生的章节快照，审核人员可以查看正文、差异、审核记录，并将通过结果推进到作品管理和发布链路。

### 采集运营

**截图占位区 3：书源采集中心**

替换文件：`demo/pc-source-center.png`

![书源采集中心：站点、策略、规则与任务](demo/pc-source-center.png)

书源采集中心按“站点授权 -> 限流策略 -> 解析规则 -> 采集任务 -> Worker 运行 -> 日志与快照 -> 审核发布”组织，任务支持主任务、子任务、备用书源、每日额度和熔断重试。

**截图占位区 4：采集数据大盘**

替换文件：`demo/pc-source-dashboard.png`

![采集数据大盘：运行指标与小说下钻](demo/pc-source-dashboard.png)

采集数据大盘从任务、站点、作品、书籍、章节、请求、错误和日志多个维度聚合，并支持进入具体小说查看当前任务、章节完整度和失败原因。

**截图占位区 5：分类与榜单管理**

替换文件：`demo/pc-ranking.png`

![分类与榜单管理：全部分类和榜单配置](demo/pc-ranking.png)

分类与榜单页面维护实际作品分类以及畅销榜、飙升榜、完结榜、新书榜和手工榜单，为 H5 书城提供可运营的数据入口。

> 替换截图时请保留文件名；完整截图建议包含浏览器窗口内的页面布局，不要截取桌面或其他应用窗口。更多截图说明见 [`demo/README.md`](demo/README.md)。
